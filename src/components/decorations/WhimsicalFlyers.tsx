...
const FLYERS = [
  ...
  { emoji: "🌈", left: "78%", top: "10%", size: "text-4xl", duration: 22, x: 6, y: -4 },
  ...
  { emoji: "🌈", left: "8%", top: "12%", size: "text-3xl", duration: 24, x: 4, y: 3 },
  { emoji: "📼", left: "84%", top: "76%", size: "text-xs", duration: 18, x: -4, y: -4 },
  { emoji: "🌈", left: "48%", top: "84%", size: "text-3xl", duration: 21, x: -3, y: -5 },
  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden>
      {FLYERS.map((item, i) => (
        <motion.span
          key={`${item.emoji}-${i}`}
          className={`absolute drop-shadow-[0_2px_4px_rgba(61,47,34,0.2)] ${
            item.emoji === "🌈" ? "opacity-95" : "opacity-50 dark:opacity-40"
          } ${item.size}`}
